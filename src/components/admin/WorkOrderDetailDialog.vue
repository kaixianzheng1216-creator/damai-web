    ...
    >
      ...
      <div class="min-h-0 flex-1 overflow-y-auto pr-1">
        ...
        <div v-else class="space-y-4">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              <icon-lucide-message-circle class="size-4 text-muted-foreground" />
              <span class="text-muted-foreground">用户未读</span>
              <span class="font-medium">{{ workOrder.userHasUnread ? '是' : '否' }}</span>
            </div>
          </div>
          ...
          <section class="space-y-3">
            ...
            <div v-else class="space-y-3">
              <div
                ...
                :key="reply.id"
                class="flex"
                :class="isUserReply(reply) ? 'justify-start' : 'justify-end'"
              >
                <div
                ...
                >
                  <div
                    ...
                    :class="
                      isUserReply(reply) ? 'text-muted-foreground' : 'text-primary-foreground/80'
                    "
                  >
                    <span>{{ isUserReply(reply) ? '用户' : '客服' }}</span>
      ...
      <template v-if="workOrder && !isLoading">
        <div v-if="!isClosed" class="space-y-2">
          <Label for="admin-work-order-reply">客服回复</Label>
          <textarea
            id="admin-work-order-reply"
        ...
        >
          ...
          <Button v-if="!isClosed" :disabled="!canReply" @click="emit('reply')">
